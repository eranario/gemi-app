...
import { AlertCircle, ChevronDown, ChevronUp, Loader2, MousePointer, Move, Upload } from "lucide-react"
...
interface OrthoInfo {
  ...
  existing_grid_settings: { options: GridOptions; offset: { lon: number; lat: number } } | null
}
...
const TARGET_COLS = [
  { key: "row",       label: "Row",       required: true,  hint: "Grid row number (1, 2, 3…)" },
  { key: "col",       label: "Column",    required: true,  hint: "Grid column number (1, 2, 3…)" },
  { key: "plot",      label: "Plot ID",   required: false, hint: "Plot label or identifier" },
  { key: "accession", label: "Accession", required: false, hint: "Entry / variety name" },
] as const
...
function remapAndSerialize(rows: Record<string, string>[], mapping: Partial<Record<TargetKey, string>>): string {
  if (rows.length === 0) return ""

  const usedSources = new Set(Object.values(mapping).filter(Boolean) as string[])
  const firstRow = rows[0]
  const passthroughCols = Object.keys(firstRow).filter((c) => !usedSources.has(c))
...
function autoDetect(headers: string[], key: TargetKey): string {
  const aliases: Record<TargetKey, string[]> = {
    ...
    accession: ["accession", "acc", "entry", "variety", "genotype", "label", "treatment"],
  }
...
function FieldDesignUploadDialog({
  ...
  function handleClose() {
    setStep("upload"); setHeaders([]); setParsedRows([]); setMapping({})
    onClose()
  }
      ...
      <DialogContent className="max-w-xl">
        ...
        {step === "map" && (
          <div className="space-y-4">
            <table className="w-full text-sm">
              <thead>
                <tr className="text-left text-muted-foreground border-b">
                  <th className="pb-2 font-medium w-1/3">Pipeline field</th>
                  ...
                  <tr key={t.key}>
                    <td className="py-2 pr-4">
                      <div className="flex items-center gap-1.5">
                        <code className="text-xs bg-muted px-1 py-0.5 rounded">{t.key}</code>
                        {t.required && <span className="text-red-500 text-xs">*</span>}
                      </div>
                    ...
                    <td className="py-2">
                      ...
                      >
                        ...
                        {t.required && <option value="">— select column —</option>}
                        {headers.map((h) => (
            ...
            <div>
              <p className="text-xs text-muted-foreground mb-1">Preview (first 4 rows after mapping):</p>
              <div className="overflow-x-auto rounded border bg-muted/40">
...
function rotateRing(coords: number[][], cx: number, cy: number, angleDeg: number): number[][] {
...
function computeGrid(
  ...
  for (let i = 0; i < rows; i++) {
    for (let j = 0; j < cols; j++) {
      ...
      if (angle !== 0) ring = rotateRing(ring, cx + offset.lon, cy + offset.lat, angle)

      const rowStr = String(i + 1), colStr = String(j + 1)
...
function deriveGridOptionsFromGeojson(fc: GeoJSON.FeatureCollection): Partial<GridOptions> {
  ...
  const lengthDeg = Math.hypot(ring[2][0] - ring[1][0], ring[2][1] - ring[1][1])

  // Average centroid latitude for metre conversion
  const allPts = features.flatMap((f) => (f.geometry as GeoJSON.Polygon).coordinates[0].slice(0, -1))
  ...
  if (f12) {
    ...
    horizontalSpacing = Math.max(0, Math.round(((Math.hypot(cx12 - cx11, cy12 - cy11) - widthDeg) / scale) * 10) / 10)
  }
  ...
  if (f21) {
    ...
    verticalSpacing = Math.max(0, Math.round(((Math.hypot(cx21 - cx11, cy21 - cy11) - lengthDeg) / scale) * 10) / 10)
  }

  // Angle: direction of the bottom edge from horizontal
  const angleRad = Math.atan2(ring[1][1] - ring[0][1], ring[1][0] - ring[0][0])
  const angle = Math.round((((angleRad * 180) / Math.PI + 360) % 360) * 10) / 10
...
function GridSettingsPanel({
  ...
  return (
    <div className="absolute bottom-4 left-4 z-[1000] bg-background/95 border rounded-lg shadow-lg w-64">
      {/* Header */}
...
export function PlotBoundaryPrep({ runId, onCancel }: PlotBoundaryPrepProps) {
  ...
  const [loadedGeojson, setLoadedGeojson] = useState<GeoJSON.FeatureCollection | null>(null)
  ...
  useEffect(() => {
    if (fdInfo?.available && fdInfo.row_count > 0 && !orthoInfo?.existing_geojson) {
      setGridOptions((prev) => ({
        ...prev,
  ...
  useEffect(() => {
    ...
    import("@geoman-io/leaflet-geoman-free").then(() => {
      ...
      const imgSrc = apiUrl(`/api/v1/pipeline-runs/${runId}/mosaic-preview`)
      ...
      if (orthoInfo.existing_geojson) {
        ...
        if (orthoInfo.existing_grid_settings) {
          ...
          if (features.length > 0) {
            ...
            const derived = deriveGridOptionsFromGeojson(orthoInfo.existing_geojson)
      ...
      if (mapAny.pm) {
        ...
        const syncBoundary = () => {
          ...
          if (hasAny) setPopBoundary((layers[0] as any).toGeoJSON() as GeoJSON.Feature)
      ...
      if (orthoInfo.existing_pop_boundary) {
        setPopBoundary(
          orthoInfo.existing_pop_boundary.type === "Feature"
            ...
            : (orthoInfo.existing_pop_boundary as GeoJSON.FeatureCollection).features[0]
        )
      }
  ...
  useEffect(() => {
    ...
    const fc = computeGrid(popBoundary, debouncedOptions, gridOffset, fdInfo?.rows)
  ...
  useEffect(() => {
    ...
    previewGeoJson.features.forEach((feature, idx) => {
      ...
      const leafletLayer = L.geoJSON(feature as GeoJSON.GeoJsonObject, {
        style: { color: "#2563eb", weight: 1.5, fillColor: "#2563eb", fillOpacity: 0.15 },
    ...
    function onMouseMove(e: L.LeafletMouseEvent) {
      ...
      if (rafId === null) {
        rafId = requestAnimationFrame(() => {
          ...
          pendingOffset.lon = 0; pendingOffset.lat = 0
          setGridOffset((prev) => ({ lon: prev.lon + lon, lat: prev.lat + lat }))
  ...
  useEffect(() => {
    ...
    function onMouseMove(e: L.LeafletMouseEvent) {
      ...
      const dist = Math.hypot(curPt.x - startContainerPt.x, curPt.y - startContainerPt.y)
  ...
  async function handleSave() {
    ...
    setIsSaving(true)
    try {
      const res = await fetch(apiUrl(`/api/v1/pipeline-runs/${runId}/save-plot-grid`), {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${localStorage.getItem("access_token") || ""}`,
      ...
      // Mark current grid as "loaded from backend" so recompute doesn't overwrite it
      setLoadedGeojson(previewGeoJson)
      // Bust cache so re-opening the tool loads the new saved grid
      await queryClient.invalidateQueries({ queryKey: ["orthomosaic-info", runId] })
      showSuccessToast("Plot boundaries saved")
    } catch {
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex items-center justify-between rounded-md border px-3 py-2 text-sm">
        {fdInfo?.available ? (
          ...
          <span className="text-red-600 font-medium flex items-center gap-1">
            ⚠ Field design required — upload one to draw plot boundaries and merge accession data.
          </span>
        )}
      ...
      <ol className="rounded-md border bg-muted/40 px-4 py-3 text-sm space-y-1.5 list-decimal list-inside">
        <li>Click the <strong>polygon icon</strong> (⬠) in the <strong>top-left toolbar</strong> to start drawing.</li>
        <li>Click around the outer edge of your field. <strong>Double-click</strong> to finish.</li>
        <li>The grid appears instantly — adjust dimensions in the <strong>Plot Grid Settings</strong> panel (bottom-left).</li>
        <li>Use <strong>Move Grid</strong> mode to drag the grid into position.</li>
        <li>In <strong>Select</strong> mode: <strong>click</strong> a plot to select it, <strong>Shift+click</strong> to add/remove, or <strong>drag</strong> a rectangle to multi-select. Red = selected. Hover for tooltip.</li>
        <li>Click <strong>Done — Save &amp; Close</strong> when satisfied.</li>
      </ol>
      ...
      <div className="relative" style={{ isolation: "isolate" }}>
        ...
        {hasBoundary && (
          <GridSettingsPanel
            ...
            onSelectAll={() => setSelectedIndexes(Array.from({ length: featureCount }, (_, i) => i))}
        ...
        {!hasBoundary && (
          <div className="absolute bottom-4 left-1/2 -translate-x-1/2 bg-background/90 border rounded-lg px-4 py-2 text-sm shadow pointer-events-none">
            Use the <strong>polygon tool</strong> (top-left) to draw the outer field boundary
          </div>
        )}
      </div>
      ...
      <div className="flex gap-2">
        ...
        {previewGeoJson && (
          <Button className="flex-1" onClick={handleSave} disabled={isSaving}>
            {isSaving ? <><Loader2 className="w-3 h-3 mr-1 animate-spin" />Saving…</> : "Save"}
          </Button>
        )}
      </div>
